import { Shield, Award, Gem, Users, Heart, Target } from 'lucide-react'
        ...
        <div className="container">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div className="relative">
              ...
              <div className="absolute -bottom-6 -right-6 bg-gold p-8">
                <p className="font-display text-4xl text-black">30+</p>
                <p className="text-xs uppercase tracking-wider text-black/70">Tahun Melayani</p>
              </div>
            </div>
        ...
        <div className="container">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {values.map((value, index) => (
              <div key={index} className="text-center p-8 border border-zinc-800">
              ...
              <div key={index} className="flex gap-8 mb-12 last:mb-0">
                <div className="flex flex-col items-center">
                  <div className="w-16 h-16 bg-gold flex items-center justify-center flex-shrink-0">
                    <span className="font-display text-lg text-black">{milestone.year}</span>
                  </div>
                  {index < milestones.length - 1 && (
                    <div className="w-px flex-1 bg-zinc-800 mt-4" />
                  )}
                </div>
                <div className="pt-3">
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-4xl mx-auto">
            {team.map((member, index) => (
              <div key={index} className="text-center group">
                <div className="aspect-square overflow-hidden mb-6 border border-zinc-800">
                  <img
                    src={member.image}
                    alt={member.name}
      ...
      <section className="py-24 bg-gold">
        <div className="container">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            <div>
              <p className="font-display text-5xl text-black mb-2">30+</p>
              <p className="text-sm uppercase tracking-wider text-black/70">Tahun Pengalaman</p>
            </div>
            <div>
              <p className="font-display text-5xl text-black mb-2">5000+</p>
              <p className="text-sm uppercase tracking-wider text-black/70">Pelanggan Puas</p>
            </div>
            <div>
              <p className="font-display text-5xl text-black mb-2">1000+</p>
              <p className="text-sm uppercase tracking-wider text-black/70">Koleksi Tersedia</p>
            </div>
            <div>
              <p className="font-display text-5xl text-black mb-2">100%</p>
              <p className="text-sm uppercase tracking-wider text-black/70">Emas Asli</p>
            </div>
          </div>
        </div>
      </section>
